...
-(void)preConfigureCell:(ZMMessage*)message{
    ...
    NSString *username = [[self.imMessage.fromBareJID componentsSeparatedByString:@"@"] objectAtIndex:0];
    self.friendInfo = [RLFriend MR_findFirstByAttribute:@"username" withValue:username];
...
//    }
    ...
    if (!message.outgoing && [message isGroupChat]) {
        NSURL *url = [NSURL URLWithString:[message avatar]];
