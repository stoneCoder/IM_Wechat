...
//    NSString *nickname = user.nickname.length ?user.nickname : _friendJID.user;
    ...
    NSString *nickname;
    if ([self.type isEqualToString:@"chat"]) {// 好友聊天
        ...
        room = [RLRoom MR_findFirstByAttribute:@"roomName" withValue:self.friendJID.user];
        if (room.roomNaturalLanguageName.length) {
            nickname = room.roomNaturalLanguageName;
        }else {
            nickname = _friendJID.user;
        }
    }
