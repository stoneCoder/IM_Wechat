...
-(void)joinAllRoom
{
    ...
    [[RLGroupLogic sharedRLGroupLogic] getGroupListByName:[jid bare] success:^(id json) {
        __block XMPPRoomManager *xmppRoomManager = nil;
        NSDictionary * dic=(NSDictionary *)json;
...
//                [room setValuesForKeysWithDictionary:roomList[i]];
            }
//        }];
...
-(void)changeNicknameInRooms:(NSString*)newName withOldName:(NSString *)oldName{
    XMPPJID * jid= xmppStream.myJID;
    [[RLGroupLogic sharedRLGroupLogic] getGroupListByName:[jid bare] success:^(id json) {
        NSDictionary * dic=(NSDictionary *)json;
        NSArray *roomList=[dic objectForKey:@"mucroomVos"];
        for (int i = 0; i < roomList.count; i++) {
            ...
            [xmppRoomManager changeNickname:newName withOldNickname:oldName withXMPPJID:roomJID];
            
            // 更改用户在群中的昵称，写入数据库表中
            [RLRoomLogic changeNickname:newName withUserJIDStr:xmppStream.myJID.bare withRoomName:roomJID.user];
            RLRoom *room = [RLRoom MR_findFirstByAttribute:@"roomName" withValue:roomJID.user];
...
#pragma mark - Create default group for friend list
-(void)createDefaultGroup{
//    XMPPGroupCoreDataStorageObject *group = [XMPPGroupCoreDataStorageObject];
}
@end