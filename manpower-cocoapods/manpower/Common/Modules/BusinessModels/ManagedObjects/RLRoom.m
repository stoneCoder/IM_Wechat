...
+(void)syncRoomWithRoomName:(NSString*)aRoomName success:(void(^)(NSString *roomName))success failure:(void(^)(id json))failure  connectionError:(void(^)(NSError *error))connectionError{
    
    [RLRoomLogic getRoomInfo:aRoomName withUserJIDStr:[XMPPManager sharedInstance].xmppStream.myJID.bare success:^(id json){
        [MagicalRecord saveWithBlock:^(NSManagedObjectContext *localContext){
            NSDictionary *roomDic = [json objectForKey:@"mucroomVos"];
            RLRoom *room = [RLRoom MR_findFirstByAttribute:@"roomName" withValue:[roomDic objectForKey:@"roomName"]];
            if (room == nil) {
                RLRoom *room = [RLRoom MR_createInContext:localContext];
                [room setValuesForKeysWithDictionary:roomDic];
            }
            if (success) {
                success([roomDic objectForKey:@"roomName"]);
            }
        }];
    } failure:failure connectionError:connectionError];
}
...
- (void)xmppRoomDidJoin:(XMPPRoom *)sender
{
    ...
    // 通过API修在群昵称，此群昵称与XMPP协议中的不一样，只是写入服务数据库中，方便API读取。
    NSString *nickname = [RLMyInfo sharedRLMyInfo].name;
    NSString *userJID = [XMPPManager sharedInstance].xmppStream.myJID.bare;
    [RLRoomLogic changeNickname:nickname withUserJIDStr:userJID withRoomName:sender.roomJID.user];
}
