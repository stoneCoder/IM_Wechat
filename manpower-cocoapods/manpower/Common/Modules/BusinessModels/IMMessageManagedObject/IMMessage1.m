...
-(void)setStringProperty:(NSString*)value withSelector:(SEL)selector{
    ...
    NSLog(@"====================Encrypt cost: %f",[afterDate timeIntervalSince1970] - [beforeDate timeIntervalSince1970]);
}
...
-(NSString*)stringProperty:(SEL)selector{
    ...
    NSLog(@"====================Decrypt cost: %f",[afterDate timeIntervalSince1970] - [beforeDate timeIntervalSince1970]);
    return retString;
}
 ...
 *
 *  @return BOOL
 *  @abstract 发送图片之前先将图片统一转成JPG格式并按30%(按微信压缩图片的测试结果估计而来)的标准来压缩图片。
 */
-(void)sendImage:(UIImage*)image withParameters:(NSDictionary*)parameters{
    if (image) {
        ...
        //        imgExt = @"jpg";
        NSString *imgTmpFileName = [NSString stringWithFormat:@"%d",(int)[[NSDate date] timeIntervalSince1970]];
        
        self.localFilePath = [NSString stringWithFormat:@"%@/%@.%@",TMP_DIR_PATH,imgTmpFileName,imgExt];
    ...
    [manager POST:urlStr parameters:parameters progress:^(NSUInteger bytesWritten, NSInteger totalBytesWritten, NSInteger totalBytesExpectedToWrite){
