...
//        self.navigationItem.rightBarButtonItem = _doneButton;
    ...
    _checkmarkView = [[QBAssetsCollectionCheckmarkView alloc] initWithFrame:CGRectMake(0, 0, 24.0, 24.0)];
    _checkmarkView.delegate = self;
//    _checkmarkView.bounced = NO;
    UIView *rightBarButtonView = [[UIView alloc] initWithFrame:_checkmarkView.frame];
    [rightBarButtonView addSubview:_checkmarkView];
    UIBarButtonItem *rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:rightBarButtonView];
//    rightBarButtonItem.customView.backgroundColor=[UIColor blueColor];
//    rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Select" style:UIBarButtonItemStyleBordered target:self action:nil];
...
-(void)setupToolbarItems{
    ...
    if (self.collectionViewController.imagePickerController.sendBarButtonItemNormalImage) {
        UIButton *sendButton = [UIButton buttonWithType:UIButtonTypeCustom];
        sendButton.frame = CGRectMake(0, 0, 60, 40);
//        [sendButton setBackgroundColor:UIColorFromRGB(0x000000)];
        [sendButton setImage:self.collectionViewController.imagePickerController.sendBarButtonItemNormalImage forState:UIControlStateNormal];
        if (self.collectionViewController.imagePickerController.sendBarButtonItemHighlightedImage) {
            [sendButton setImage:self.collectionViewController.imagePickerController.sendBarButtonItemHighlightedImage forState:UIControlStateHighlighted];
        }
        [sendButton addTarget:self.collectionViewController action:@selector(done:) forControlEvents:UIControlEventTouchUpInside];
        sendButtonItem = [[QBAssetsBarButtonItem alloc] initWithCustomView:sendButton];
    }else{
        sendButtonItem = [[QBAssetsBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"send",@"QBImagePickerController",nil) style:UIBarButtonItemStyleBordered target:self.collectionViewController action:@selector(done:)];
    }
    ...
    NSArray *toolbarItems = @[spaceButtonItem, sendButtonItem];
    
    [_toolbar setItems:toolbarItems];
    if (self.collectionViewController.imagePickerController.toolbarBackgroundImage) {
        [_toolbar setBackgroundImage:self.collectionViewController.imagePickerController.toolbarBackgroundImage forToolbarPosition:UIBarPositionBottom barMetrics:UIBarMetricsDefault];
    }
...
-(void)updateToolbar{
    BOOL enable = [self.collectionViewController validateNumberOfSelections:(self.collectionViewController.imagePickerController.selectedAssetURLs.count)];
    __block QBAssetsBarButtonItem *sendBBI = _toolbar.items[1];
    sendBBI.enabled = enable;
    sendBBI.badgeOriginX = 1;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        sendBBI.badgeValue = [NSString stringWithFormat:@"%d",(int)[self.collectionViewController.imagePickerController.selectedAssetURLs count]];
    });
...
- (void)updateCheckmarkView{
    ...
    NSURL *photoURL = [photo.asset valueForProperty:ALAssetPropertyAssetURL];
//    for (NSURL *url in self.collectionViewController.imagePickerController.selectedAssetURLs) {
//        if ([url isEqual:photoURL]) {
//            NSLog(@"equal");
//        }
//    }
    
    if ([self.collectionViewController.imagePickerController.selectedAssetURLs containsObject:photoURL]) {
        _checkmarkView.checked = YES;
    }else{
        _checkmarkView.checked = NO;
    }
...
-(BOOL)checkmarkViewShouldCheck:(QBAssetsCollectionCheckmarkView *)checkmarkView{
    ...
    return [self.collectionViewController validateNumberOfSelections:(self.collectionViewController.imagePickerController.selectedAssetURLs.count + 1)];
    //    }
    //    return YES;
}
...
-(void)checkmarkViewDidCheck:(QBAssetsCollectionCheckmarkView *)checkmarkView{
    MWPhoto *photo = [self photoAtIndex:self.currentIndex];
    if (self.collectionViewController.delegate && [self.collectionViewController.delegate respondsToSelector:@selector(assetsCollectionViewController:didSelectAsset:)]) {
        [self.collectionViewController.delegate assetsCollectionViewController:self.collectionViewController didSelectAsset:photo.asset];
    }
    
    [self.collectionViewController updateToolbar];
    [self updateToolbar];
}
...
-(void)checkmarkViewDidUncheck:(QBAssetsCollectionCheckmarkView *)checkmarkView{
    ...
    // Delegate
    if (self.collectionViewController.delegate && [self.collectionViewController.delegate respondsToSelector:@selector(assetsCollectionViewController:didDeselectAsset:)]) {
        [self.collectionViewController.delegate assetsCollectionViewController:self.collectionViewController didDeselectAsset:photo.asset];
    }
    [self.collectionViewController updateToolbar];
    [self updateToolbar];
}
