...
-(void)setupHeaderView{
    ...
    [self addBottomBorderWithColor:UIColorFromRGB(0xcccccc) andWidth:0.5];
...
-(void)headerViewTapped:(UITapGestureRecognizer*) tapGestureRecognizer{
    if ([self.titleTextField isFirstResponder]) {
        ...
        __block NSString* newName = [self.titleTextField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        __block NSString* oldName = myInfo.name;
        @weakify(self);
        if (newName.length > 0 && ![newName isEqualToString:myInfo.name]) {
            myInfo.name = newName;
