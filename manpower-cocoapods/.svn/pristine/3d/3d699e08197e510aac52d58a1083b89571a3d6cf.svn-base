...
-(void)changePhotoWithImage:(UIImage *)image
{
    ...
    if ([IMMessageHelper saveImage:image withFullFilePath:fullPath]) {
        NSString *roomName = _group.roomName != nil ? _group.roomName:_room.roomName;
        [RLRoomLogic updatePhoto:fullPath withRoomName:roomName withView:self.viewController.view success:^(id json){
            DDLogVerbose(@"%@",json);
        } failure:^(id json){
